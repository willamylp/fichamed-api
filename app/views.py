    ...
    def get(self, request, format=None):
        return Response({
            ...
            'Authentication': {
                ...
                'token_blacklist': reverse('token_blacklist', request=request, format=format),
            },
            'Accounts': {
                'users_list_create': reverse('users_list_create', request=request, format=format),
                'user_datail_view/<int:pk>': reverse(
                    'user_datail_view',
                    kwargs={'pk': 1},
                    request=request,
                    format=format
            ...
            'AttendanceSheet': {
                ...
                'attendance_sheet_datail_view/<int:pk>': reverse(
                    'attendance_sheet_datail_view',
                    kwargs={'pk': 1},
                    request=request,
                    format=format
                ),
            ...
            'MedicalCareHistory': {
                ...
                'medical_history_datail_view/<int:pk>': reverse(
                    'medical_history_datail_view',
                    kwargs={'pk': 1},
                    request=request,
                    format=format
                ),
